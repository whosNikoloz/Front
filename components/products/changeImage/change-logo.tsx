...
function ChangeProductLogo({
  ...
  Productid: number;
  ProductName: string;
  onUpdateLogo(newPicture: string, ProductId: number): void;
}) {
  const {
  ...
  const handleSavePicture = async () => {
    ...
    if (newpicture) {
      ...
      const response: ApiResponse<any> = (await ProductAPi.ChangeProductLogo(
        file,
        oldpicture,
        Productid,
        ProductName
      ...
      if (response.status) {
        onUpdateLogo(response.result, Productid);
            ...
            <>
              ...
              <ModalBody className="flex flex-col items-center gap-6 ">
                <label
                  ...
                  onDrop={handleDrop}
                >
                  <Avatar src={ProductLogo} className="w-28 h-28 " />
          ...
          {(onCropModalClose) => (
            <>
              ...
              <ModalBody className="flex flex-col items-center py-6">
                <CropImg
                  ProductLogo={UploadedImg || ProductLogo}
...
export default ChangeProductLogo;
